...
class MyAir:
    ...
    def _request(self, request):
        ...
        url = "http://%s/%s" % (self.host, request)
    ...
    def login(self):
        try:
            url = "http://%s/login?password=password" (self.host)
            response = requests.get(url, timeout=5)
            response.raise_for_status()

            root = ElementTree.fromstring(response.text)
            auth = root.find('.//authenticated')
            return auth is not None and auth.text == '1'
    ...
    def getZoneCount(self, systemdata=None):
        self.login
    ...
    def getSystem(self):
        ...
        print(req.get('iZS10.3').get('system'))
        return req.get('iZS10.3').get('system')
        

    def getFanSpeed(self, systemdata=None):
        self.login
        if systemdata is None:
            systemdata = self.getSystem()
        fanspeed = systemdata.get('unitcontrol').get('fanSpeed')
        try:
            fanspeed_name = list(FAN_MAP.keys())[list(FAN_MAP.values()).index(int(fanspeed))]
            return fanspeed_name
        except (ValueError, IndexError):
            return 'unknown'
        

    def getMode(self, systemdata=None):
        self.login
        if systemdata is None:
            systemdata = self.getSystem()
        ...
        if onoff == '0':
            ...
            try:
                mode_name = list(MODE_MAP.keys())[list(MODE_MAP.values()).index(int(mode))]
                return mode_name
            except (ValueError, IndexError):
                return 'unknown'
